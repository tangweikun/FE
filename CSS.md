...
> <h3>定义：</h3>BFC(Block formatting context)直译为"块级格式化上下文"。它是一个独立的渲染区域，只有 Block-level box 参与， 它规定了内部的 Block-level Box 如何布局，并且与这个区域外部毫不相干

> <h3>触发条件：</h3>根元素；float 属性不为 none；position 为 absolute 或 fixed；display 为 inline-block, table-cell, table-caption, flex, inline-flex；overflow 不为 visible
