...
1.  [`DocType`有什么作用?严格模式和混杂模式的区别](#DocType有什么作用?严格模式和混杂模式的区别)
1.  [介绍一下你对浏览器内核的理解?](#介绍一下你对浏览器内核的理解)
1.  [常见的浏览器内核有哪些?](#常见的浏览器内核有哪些)
1.  [常见的浏览器`Javascript`引擎有哪些?](#常见的浏览器Javascript引擎有哪些)
1.  [为什么最好把`CSS`的`<link>`标签放在`<head></head>`之间?为什么最好把`JS`的`<script>`标签恰好放在`</body>`之前?](#为什么最好把CSS的link标签放在headhead之间?为什么最好把JS的script标签恰好放在body之前)
